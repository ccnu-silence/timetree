...
//		videoInfo.setNotifyUrl(ConfigUtil.NOTIFY_URL);
		...
		// 通知Upload receiver
		Intent broadCastIntent = new Intent(AppConstants.ACTION_UPLOAD);
	...
	UploadListener uploadListenner = new UploadListener() {
		...
		public void handleStatus(VideoInfo v, int status) {
			videoInfo = v;
			Intent intent = new Intent(AppConstants.ACTION_UPLOAD);
...
//            Log.i("upload service", "p  = "+ p );
			if (progress <= 100) {
				...
				if (progress % 10 == 0) {
                    ...
                    Log.i("upload service", "progress  = "+ p );
				}
