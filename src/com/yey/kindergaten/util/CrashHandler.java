...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread thread, Throwable ex) {
        ...
        LogErrorFollowing.w(TAG,"uncaughtException ,and thread or Throwable is : "
                + thread.getName() + " / " + ex.getMessage() );
        UtilsLog.w(TAG,"uncaughtException ,and thread or Throwable is : "
                + thread.getName() + " / " + ex.getMessage() );
...
//	        android.os.Process.killProcess(android.os.Process.myPid());
	...
	public boolean handleException(Thread thread,final Throwable ex) {
        ...
        LogErrorFollowing.w(TAG,"handleException : " + ex.getMessage());
        UtilsLog.w(TAG,"handleException : " + ex.getMessage());
        ...
        if (ex.getCause().getStackTrace()!=null && ex.getCause().getStackTrace().length!=0) {
            if (ex.getCause().getCause()!=null) {
                ...
                LogErrorFollowing.w(TAG,"ex.getcause is null");
                UtilsLog.w(TAG,"ex.getcause is null");
            }
            ...
            for (int i = 0; i < ex.getCause().getStackTrace().length; i++) {
                if (ex.getCause().getStackTrace()[i]!=null) {
                    ...
                    LogErrorFollowing.w(TAG,"ex.getStackTrace is null");
                    UtilsLog.w(TAG,"ex.getStackTrace is null");
                }
            }
            LogErrorFollowing.w(TAG,"handleException : " + buffer.toString());
            UtilsLog.w(TAG,"handleException : " + buffer.toString());
	...
	private File save2File(String crashReport) {
		...
		if (Environment.getExternalStorageState().equals(
			...
			try {
                ...
                LogErrorFollowing.i("CrashHandler","save2File error:" + e.getMessage());
                UtilsLog.i("CrashHandler","save2File error:" + e.getMessage());
			}
		}
		return null;
	}
	...
	private PackageInfo getPackageInfo(Context context) {
		...
		try {
			...
			UtilsLog.i("CrashHandler","getPackageInfo err = " + e.getMessage());
		}
		if (info == null)
