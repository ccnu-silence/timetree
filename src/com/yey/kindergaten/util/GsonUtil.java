...
public class GsonUtil {
	...
	public static String listToJson(List<?> l) {
		...
		String beanListToJson = gson.toJson(l, type);
	 ...
	 * @return
	 */
	public static List<?> jsonToListByKey(String jsonStr,String key,java.lang.reflect.Type type) {
		Object rulsObj = null;
		Map<?, ?> rulsMap = jsonToMap(jsonStr);
		if (rulsMap != null && rulsMap.size() > 0) {
			rulsObj = rulsMap.get(key);
		...
		List<?> objList = null;
		if (gson != null) {
			objList = gson.fromJson((String)rulsObj, type);
		}
		return objList;
	}
