...
public class AlbumHelper {
	...
	HashMap<String, ImageDir> bucketList = new HashMap<String, ImageDir>();
...
//						+ bucketName + "---");
				...
				ImageDir bucket = bucketList.get(bucketId);
				if (bucket == null) {
					bucket = new ImageDir();
		...
		Iterator<Entry<String, ImageDir>> itr = bucketList.entrySet()
				.iterator();
		while (itr.hasNext()) {
			Map.Entry<String, ImageDir> entry = (Map.Entry<String, ImageDir>) itr
					.next();
			ImageDir bucket = entry.getValue();
...
//						+ ", " + image.thumbnailPath);
	 ...
	 * @return
	 */
	public List<ImageDir> getImagesBucketList(boolean refresh) {
		if (refresh || (!refresh && !hasBuildImagesBucketList)) {
			buildImagesBucketList();
		}
		List<ImageDir> tmpList = new ArrayList<ImageDir>();
		Iterator<Entry<String, ImageDir>> itr = bucketList.entrySet()
				.iterator();
		while (itr.hasNext()) {
			Map.Entry<String, ImageDir> entry = (Map.Entry<String, ImageDir>) itr
